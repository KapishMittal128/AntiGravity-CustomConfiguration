...
- Every skill MUST have a YAML frontmatter block with `name`, `description`, `version`, `verified_date`, and `category`.
...
- `description` should include the phrase `Use when` followed by concrete triggers.
- Every skill MUST have a "When to Use This Skill" section.
- Every skill MUST have a "Purpose" section.
- Every skill MUST have an "Output Format / Delivery" section.
- Every skill MUST have a "Behavior Rules" section at the bottom.
- Every skill MUST include a short "Maintenance Notes" section.
