...
Use each file for one specific job:
- `AGENTS.md` — top-level policy, routing, safety, and delivery rules
- `skills/RUNTIME_CONTROL.md` — runtime checkpoints, phase lockouts, and state synchronization
- `skills/SYSTEM_ORCHESTRATION.md` — multi-skill decomposition, handoffs, and sequencing
- `skills/CAPABILITIES.md` — live skill routing index
- `settings.json` — machine-readable config mirror for commands, behavior flags, and directories
...
1. Is this Single-Skill or Multi-Skill (see `.agents/skills/SYSTEM_ORCHESTRATION.md`)?
...
- You hit a `.agents/skills/RUNTIME_CONTROL.md` checkpoint and need to declare a hard stop.
...
→ Load: repo-analysis (primary), backend-dev-guidelines (supporting)
...
→ Load: database-design (primary only — it covers the schema and index reasoning available in this workspace)
→ Stop. Do NOT also load unrelated backend or frontend skills.
...
State management is explicitly governed by the execution rules in `.agents/skills/RUNTIME_CONTROL.md`.
...
When a user prompt begins with a `/` followed by a known command name, treat it as an automatic command invocation.
...
powershell -ExecutionPolicy Bypass -File .agents/hooks/*.ps1
```
...
- `.agents/skill-system/import-playbook.md` — how to import skills from external sources
- `.agents/skill-system/conversion-guide.md` — how to convert Claude/generic prompts to Antigravity format
- `.agents/skill-system/skill-rules.md` — quality and freshness standards for skills
- `.agents/skill-system/description-guide.md` — how to write discoverable skill descriptions
- `.agents/skill-system/category-map.md` — what goes where
- `.agents/skill-system/skill-template.md` — canonical blank template with version/verified_date fields
- `.agents/skill-system/maintenance-guide.md` — protocol for refreshing skills when they become stale
...
When writing state updates, always write to the **project root** copy, never to `.agents/STATE.md`.
...
State:              Sync STATE.md + task.md per .agents/skills/RUNTIME_CONTROL.md rules
```
